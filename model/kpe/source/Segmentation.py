...
def get_basic_stop_words_file():
    d = os.path.dirname(os.path.realpath(__file__))
    return os.path.join(d, '../data/basic_stopwords.txt')
...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words_file=None, allow_speech_tags=util.allow_speech_tags, user_dict=None):
        ...
        if user_dict:
            ...
            #jieba.load_userdict(os.path.join(d,'data/cnki/keywords_dict.txt'))
        ...
        # self.basic_stop_words = set()
        # self.basic_stop_words_file = get_basic_stop_words_file()
        # for word in codecs.open(self.basic_stop_words_file, 'r', 'utf-8', 'ignore'):
        #     self.basic_stop_words.add(word.strip())
    ...
    def segment(self, text, lower=True, use_stop_words=True, use_speech_tags_filter=False):
        ...
        if use_stop_words:
            ...
            word_list = [w.word.strip() for w in jieba_result if w.flag != 'x']
            #word_list = [word.strip() for word in word_list if word.strip() not in self.basic_stop_words]
...
class Segmentation(object):
    ...
    def simple_segment(self, text, lower=False):
        text = util.as_text(text)
        sentences = self.ss.segment(text)

...
def parse_args():
    ...
    parser.add_argument("--file_dir", type=str, default=Valid_dataset_file_dir[Dataset_na])
    ...
    # parser.add_argument("--model_out", type=str, default="../model/JointMarkScoreKpe/cnki_2/out", help="model output path")
    # parser.add_argument("--scorekpe_model", type=str, default="../model/JointMarkScoreKpe/cnki_2/out/epoch_2")
    parser.add_argument("--user_dict", type=str, default='../data/cnki_2/keywords_dict.txt')
...
def train(args):
    ...
    for epoch_i in range(basic, basic + epochs):
        ...
        total_train_loss = 0
        total_train_mark_loss = 0
        total_train_rank_loss = 0
        loss_batch = 0
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            total_train_mark_loss += mark_loss.item()
            total_train_rank_loss += rank_loss.item()
        ...
        avg_train_mark_loss = total_train_mark_loss / len(train_dataloader)
        avg_train_rank_loss = total_train_rank_loss / len(train_dataloader)
        ...
        print("  Average training  mark loss: {0:.2f}".format(avg_train_mark_loss))
        print("  Average training rank loss: {0:.2f}".format(avg_train_rank_loss))
        ...
        total_eval_loss = 0
        total_eval_mark_loss = 0
        total_eval_rank_loss = 0
        nb_eval_steps = 0
        ...
        for batch in tqdm(validation_dataloader):
            ...
            total_eval_mark_loss += mark_loss.item()
            total_eval_rank_loss += rank_loss.item()
        ...
        avg_val_mark_loss = total_eval_mark_loss / len(validation_dataloader)
        avg_val_rank_loss = total_eval_rank_loss / len(validation_dataloader)

        # 统计本次评估的时长
        validation_time = format_time(time.time() - t0)

        print("  Validation Loss: {0:.2f}".format(avg_val_loss))
        print("  Validation Mark Loss: {0:.2f}".format(avg_val_mark_loss))
        print("  Validation Rank Loss: {0:.2f}".format(avg_val_rank_loss))
        ...
        training_stats.append(
            {
                ...
                'Training Mark Loss': avg_train_mark_loss,
                'Training Rank Loss': avg_train_rank_loss,
                'Valid. Loss': avg_val_loss,
                'Valid. Mark Loss': avg_val_mark_loss,
                'Valid. Rank Loss': avg_val_rank_loss,
...
def tesk(args):
    ...
    text = '农村留守经历大学新生自尊社会支持和应对方式与孤独感的关系。目的探讨农村留守经历大学新生自尊、社会支持、应对方式与孤独感关系,为降低农村留守经历大学新生的孤独感提供参考。方法采用UCLA孤独量表第3版、自尊评定量表（SES）、领悟社会支持量表（PSSS）、简易应对方式量表,对从贵州6所高校随机抽取的615名农村留守经历大学新生和479名农村非留守大学新生进行调查。结果农村留守经历大学新生孤独感高于农村非留守大学新生（t=3.89,P<0.01）;农村留守经历大学新生自尊、家庭支持、其他支持均低于农村非留守大学新生（t值分别为-2.28,-3.59,-3.30,P值均<0.05）;农村留守经历大学新生孤独感与自尊、社会支持各因子、积极应对方式均呈负相关（r值范围为-0.49-0.34,P值均<0.01）,与消极应对方式呈正相关（r=0.22,P<0.01）;回归分析表明,朋友支持、自尊、积极应对和消极应对可解释农村留守经历大学新生孤独感总变异的39.00%。结论留守经历、朋友支持、自尊、积极和消极应对方式对农村留守经历大学新生孤独感有重要影响。'
    ...
    #     print(c)
    mark_res = model.generateMarkAns(word_li,mark_score)
